...
export function agentsBlock(input: AgentsFileInput): string {
  ...
  const lines: string[] = [
    ...
    '5. **Define the model with the CLI, never by guessing.** Content types and object',
    '   shapes belong to THIS PROJECT, not to the SDK version — read them with',
    `   \`npx ${CLI_BIN} types list\` before assuming what exists. Writing a field the`,
    '   project has not declared is a 422, not a silent extra key.',
    '',
    '### Content types',
    '',
    ...
    '',
    'The types below are what this project holds RIGHT NOW. They are not fixed: define',
    'more from the command line, then regenerate the types file.',
    '',
    '```bash',
    `npx ${CLI_BIN} types list                  # what this project defines`,
    `npx ${CLI_BIN} templates list              # ready-made models`,
    `npx ${CLI_BIN} templates apply hospitality # add one — never overwrites`,
    `npx ${CLI_BIN} types add room \\`,
    '  --field title:string! --field slug:string! \\',
    '  --field gallery:object<galleryImage>[]',
    `npx ${CLI_BIN} types                       # regenerate the type definitions`,
    '```',
    '',
    'Changes are **additive only**, and that is what makes them free of migrations: a',
    'field may be added, none may be removed, renamed, retyped, or made required.',
    'Stored documents are never rewritten. To retire a field, keep it and mark it',
    '`deprecated` — it disappears from the studio, keeps being served by the API, and',
    'shows up struck through in your editor on the next `types` run.',
    '',
    'A type cannot be deleted while documents of it exist. Nothing in this API removes',
    'content as a side effect of a schema change.',
    '',
    '### Images',
    '',
    'Never point an `<img>` at an asset URL directly — that serves the ORIGINAL, which',
    'is whatever came out of the owner\'s camera. `imageProps` returns a signed,',
    'resized, WebP rendition with a `srcset`, the asset\'s own alt text, and a blur',
    'placeholder:',
    '',
    '```tsx',
    `import Image from 'next/image'`,
    `import { imageProps } from '${SDK_PACKAGE}'`,
    '',
    'const props = imageProps(page.heroImage, {',
    '  width: 1600, height: 1000, fit: \'crop\', priority: true,',
    '})',
    '',
    '// null when the field is empty — guard once, not per attribute.',
    'return props && <Image {...props} />',
    '```',
    '',
    '- **`fit`** is `max` (fit inside) or `crop` (exact box). A crop respects the focal',
    '  point the owner set in the studio, so it will not cut off the subject.',
    '- **`priority`** on the one image above the fold. It is the biggest single lever',
    '  on LCP and the only thing here that cannot be inferred.',
    '- **`alt`** comes from the asset. Pass one only where this usage genuinely differs.',
    '- **Reuse sizes.** Each asset allows 12 renditions; a width computed fresh per',
    '  request exhausts that and starts returning 403. Pick a few and stick to them.',
    '- Images are never upscaled, and animated GIFs are served unchanged.',
    '',
    '### Page metadata (title, description, Open Graph)',
    '',
    'Never type a `<title>` or a description into the code. Every type carries a `seo`',
    'object the owner edits in the studio; `metadataFor` turns it into a Next `Metadata`,',
    'including a 1200x630 Open Graph image built from `seo.image`:',
    '',
    '```ts',
    `import { metadataFor } from '${SDK_PACKAGE}/seo'`,
    '',
    `export const generateMetadata = metadataFor(() => getPage('home'), { path: '/' })`,
    '```',
    '',
    `- Set \`${ENV.siteOrigin}\` to the site's real origin, e.g. \`https://example.com\`.`,
    '  Without it there is no canonical URL at all, which is deliberate: a canonical',
    '  pointing at the wrong host removes the right page from search results.',
    '- **`path`** is the page\'s URL. It defaults to `/{slug}` (and to `/` for the slug',
    '  `home`), so a NESTED route has to say so, e.g. a post at `/blog/first` needs',
    '  `path: "/blog/" + slug`. A layout reading a settings singleton must pass',
    '  `path: false`, because that document is not a page and its slug is not a URL.',
    '- The OG image URL is permanent and safe for a social network to cache forever.',
    '  Rotating the project read key is the one thing that breaks already-shared cards.',
    '',
