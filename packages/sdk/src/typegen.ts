...
function augmentation(definitions: readonly ContentTypeDefinition[]): string {
  ...
  return [
    ...
    '',
    '/*',
    ' * The empty import is load-bearing. TypeScript refuses to augment a module',
    ' * the file has not imported — TS2664, "module cannot be found", raised even',
    ' * though resolution succeeds and `--traceResolution` shows it landing on the',
    ' * right `.d.ts`.',
    ' *',
    ' * It only appears in a project where nothing ELSE imports the SDK from a',
    ' * TypeScript file, which is precisely a freshly scaffolded one: the first',
    ' * `build` after `init`, before the developer has written a line. A project',
    ' * with a single real import never sees it, which is how it survived.',
    ' */',
    `import type {} from ${quote(SDK_PACKAGE)}`,
    '',
